...
class SimpleCRUD
{
    ...
    public function cellsGetAction()
    {
        ...
        $query->setSpreadsheetKey("pqDK7wzuzrlSzNne29jpKDQ");
        $query->setWorksheetId($this->currWkshtId);
   ...
   public function GetlistBasedFeed( $feedWorksheet){
   $i=0;
   	     foreach($feedWorksheet->entries as $entry) {
    		$WorksheetId=split("/",$entry->id->text);	
    		/*$query = new Zend_Gdata_Spreadsheets_CellQuery();
			$query->setSpreadsheetKey("p9ISv2bT_pua6y0c0Bu-XJA");
			$query->setWorksheetId($WorksheetId[8]);
            $feed = $this->gdClient->getCellFeed($query);
			//print($i."-".$WorksheetId[8]."</br>");
            $i++;*/
            $data[]=$entry->title->text;
    	}
