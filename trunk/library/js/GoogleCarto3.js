	...
	if (p.status != "200" ){
	      alert("Réception erreur " + p.status);
			...
			for (var m=0;m<i;m++)
			{
					...
					var n = liste[m];
					m++;
					var id_rubrique = liste[m];
					m++;
					var topic = liste[m];
					m++;
					var lien =liste[m];
					m++;
					var localisation= liste[m];
					m++;
					var image =liste[m];
					m++;
					var titre =liste[m];
					...
					switch (query)
					{
					  ...
					  default:
							if(lat!='') {
								...
								//ajoute l'événement mouse over au markers
								/*
