...
require_once(dirname(__FILE__) . "/Onadaflexconn.php");
require_once(dirname(__FILE__) . "/functions.inc.php");
require_once(dirname(__FILE__) . "/XmlSerializer.class.php"); 

//require_once ("C:/wamp/www/onadabase/trouville/param/ParamPage.php");
...
function GetQuery() {
	...
	$from = ' FROM spip_forms_donnees fd
					...
					INNER JOIN spip_forms_donnees_champs fdc10 ON fdc10.id_donnee = fd.id_donnee AND fdc10.champ = "texte_1"
					INNER JOIN spip_forms_donnees_champs fdc11 ON fdc11.id_donnee = fd.id_donnee AND fdc11.champ = "texte_2"
			';

	/*
  	Modifier   	Effacer  	70 	multiple_1 	multiple_1_1  	Réglementaire 	1
	...
	*/
	//vérifie si on fait une requête pour renvoyer les controles sans mot clef
	if($_REQUEST["idMot"]==-1)
...
function insertNew() {
	...
	//récupération de l'article où stocker la grille à partir du mots-clef
	...
	$row = array("grille"=>$idGrille,"champ"=>"texte_1","valeur"=>GetSQLValueString($_REQUEST["texte_1"], "text"));
	$g->SetChamp($row, $idDon);
	$row = array("grille"=>$idGrille,"champ"=>"texte_2","valeur"=>GetSQLValueString($_REQUEST["texte_2"], "text"));
	$g->SetChamp($row, $idDon);
...
function update() {
	...
	if(substr($_REQUEST["champ"],0,8)=='multiple'){
		...
		if($valeur=='false'){
			...
			//si elle est vrai on crée la ligne
			$toret=insert();
		}
	}
